...
function M.conflicts(root)
    local code, entries, stderr = M.status(root)
    if code ~= 0 then
        return code, {}, stderr
    end
    ...
    for _, entry in ipairs(entries) do
        if unmerged_statuses[entry.status] then
...
local function side_action(status, side)
    if status == "DD" then
        return "remove"
    end

    if status == "UD" or status == "AU" then
        return side == "ours" and "checkout" or "remove"
    end

    if status == "DU" or status == "UA" then
        return side == "ours" and "remove" or "checkout"
    end

    return "checkout"
end

function M.accept_conflict(root, entry, side)
    local action = side_action(entry.status, side)

    if action == "remove" then
        return run_git(root, { "rm", "-f", "--", entry.path })
    end

    local checkout_code, _, checkout_stderr = run_git(root, {
        "checkout",
        "--" .. side,
        "--",
        entry.path,
    ...
    if checkout_code ~= 0 then
        return checkout_code, "", checkout_stderr
    end

    return run_git(root, { "add", "--", entry.path })
end

function M.accept_conflicts(root, entries, side)
    local errors = {}

    for _, entry in ipairs(entries) do
        local code, _, stderr = M.accept_conflict(root, entry, side)
        if code ~= 0 then
            table.insert(errors, vim.trim(stderr) ~= "" and vim.trim(stderr) or entry.path)
        end
    end

    if #errors > 0 then
        return 1, table.concat(errors, "\n")
    end
